...
class CareerlinkscraperPipeline:
    def process_item(self, item, spider):
        ...
        time_update_str = adapter.get('time_update').strip()
        if time_update_str:
            try:
                # Kiểm tra định dạng "Ngày/Tháng/Năm" (ví dụ: "31/10/2024")
                adapter['time_update'] = datetime.strptime(time_update_str, "%d/%m/%Y")
            except ValueError:
                try:
                    # Kiểm tra định dạng "Năm-Tháng-Ngày" (ví dụ: "2024-10-31")
                    adapter['time_update'] = datetime.strptime(time_update_str, "%Y-%m-%d")
                except ValueError:
                    adapter['time_update'] = datetime.now()

        ## Convert time_expire to datetime
        time_expire = adapter.get('time_expire')
        if time_expire:
            try:
                adapter['time_expire'] = adapter['time_update'] + timedelta(days=time_expire)
            except (ValueError, IndexError):
                adapter['time_expire'] = datetime.now() + timedelta(days=30)  # Hoặc giá trị mặc định
