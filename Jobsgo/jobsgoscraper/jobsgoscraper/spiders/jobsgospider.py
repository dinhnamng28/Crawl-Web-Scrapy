...
from jobsgoscraper.items import JobItem
from jobsgoscraper.pipelines import DatabaseConnector
...
class JobsgospiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        connector = DatabaseConnector(server="DinhNamNguyen\DinhNam", database="jobsgo_stg")
    ...
    def parse_job_page(self, response):
        ...
        if time_expire:
            try:
                time_expire = int(time_expire)  # Giả sử `time_expire` là số ngày
            except ValueError:
                time_expire = 0  # Nếu không thể chuyển đổi sang số, đặt là 0
        else:
            time_expire = 0  # Nếu không có hạn sử dụng, mặc định là 0 ngày
